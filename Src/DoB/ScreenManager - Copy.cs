...
namespace GameManager
{
  public class ScreenManager
  {
    ...
    {
      if (/*key.IsKeyDown(Keys.Enter) && */ Game2.gamePhase == Game2.GamePhase.Intro
            && this.cooldownToPressEnter < 0)
      {
         Game2.gamePhase = Game2.GamePhase.Game;
         this.cooldownToPressEnter = 120;
      }

      if (/*key.IsKeyDown(Keys.Enter) && */Game2.gamePhase == Game2.GamePhase.HeadphonesRecommended
                && this.cooldownToPressEnter < 0)
      {
        Game2.gamePhase = Game2.GamePhase.Intro;
        this.cooldownToPressEnter = 120;
      }


      if (Game2.gameState == Game2.GameState.ChangingRoom)
      {
    ...
    public void Draw(SpriteBatch sb)
    {
        ...
        if (Game2.gamePhase == Game2.GamePhase.HeadphonesRecommended)
            sb.Draw(this.texHeadphones, new Vector2(0.0f, 0.0f), Color.White);
     
        if (Game2.gamePhase == Game2.GamePhase.Intro)
        ...
        if (Game2.gamePhase != Game2.GamePhase.GameOverSplatter  || !ScreenManager.requestGameOver)
         return;
    ...
    public void HitByMonster()
    {
      ...
      Game2.gameState = Game2.GameState.Paused;
